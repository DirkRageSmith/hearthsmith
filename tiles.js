...
(function (root, factory) {
  ...
  const PALETTE = {
    ...
    /* The character. Added 2026-09-02 — these are NEW keys, not re-used ones.
     * The freeze protects existing letters from being renamed or repurposed;
     * adding to the end costs nothing and breaks no grid. */
    "1": "#f2c8a0", // skin
    "2": "#c99b73", // skin, shadow
    "3": "#4a3040", // hair
    "4": "#4a7fd8", // shirt
    "5": "#2f5aa0"  // shirt, shadow
  };
  ...
  /* ---- YOU ---------------------------------------------------------------
   * Six frames: front, back and side, each with a step. Left is the side frame
   * mirrored at draw time rather than drawn twice — half the art, and it
   * cannot drift out of sync with itself.
   *
   * Proportions are the EarthBound register (ADR-030): a big head on a small
   * body, roughly 1:2, which is what reads as "a person" at sixteen pixels and
   * as friendly rather than heroic. A realistically-proportioned 16px figure
   * reads as a smudge — the head is doing all the work of telling you this is
   * someone rather than something.
   *
   * No face on the back frame. Faces on the back of heads are a real and very
   * common pixel-art mistake. */
  const PLAYER = {
    you_down_a: [
      "................",
      ".....333333.....",
  ...
  Object.keys(PLAYER).forEach(function (k) { SPRITES[k] = PLAYER[k]; });
